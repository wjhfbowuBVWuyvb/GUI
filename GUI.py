...
if uploaded_file is not None:
    ...
    xlim_end_signal = st.number_input("X-axis End for Signal Plot", min_value=0, max_value=signal_length, value=int(time_axis[-1] * fs), step=1)
    ax_signal.set_xlim([xlim_start_signal/fs, xlim_end_signal/fs])
    ...
    for channel_index, signal in enumerate(signals):
        ...
        # Murmur algorithm
            ...
            xlim_end_rhythm = st.number_input(f"X-axis End for Rhythm Plot (Channel {channel_index + 1})", min_value=0, max_value=len(signal), value=time_axis_downsampled, step=1)
